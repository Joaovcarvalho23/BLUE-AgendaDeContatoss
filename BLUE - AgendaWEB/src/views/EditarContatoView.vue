<template>
  <div class="editar-contato-container">
    <div class="flex align-items-center mb-4">
      ...
      />
      <div>
        <h1 class="m-0 text-900">Editar Contato</h1>
        <p class="m-0 text-500">Atualize as informações do contato</p>
      </div>
    </div>

    <div v-if="loading" class="text-center p-5 surface-card border-round-xl">
      <ProgressSpinner />
      <p class="text-500 mt-3">Carregando contato...</p>
    </div>

    <Card v-else class="surface-card shadow-2 border-round-xl">
      <template #content>
...
const contatoId = ref(Number(route.params.id))
...
const loadContato = async () => {
  try {
    const contato = await contatoService.getById(contatoId.value)

    form.nome = contato.nome
    form.email = contato.email
    form.telefone = contato.telefone
  } catch (error: any) {
    ErrorHandler.handle(error, toast, 'Erro ao carregar contato')
    router.push('/home')
  } finally {
    loading.value = false
  }
...
const handleUpdate = async () => {
  ...
  try {
    await contatoService.update(contatoId.value, {
      id: contatoId.value,
      ...form
    })
    ...
    ErrorHandler.handle(error, toast, 'Erro ao atualizar contato')
  } finally {
